...
class BoxOfficeSpider(Spider):
	...
	def parse_movie_page(self, response):
		...
		for movie in movie_info: #NOT NECESSARY(?)
			...
			# intltab_selectors = international_tab.xpath('.//div//tr') #contains selectors will all values in interational tab, now use for loop

			# # For each Selector in the table containing the rank
			# for intlselector in intltab_selectors:
			...
			# 	# Option 1:
				...
				# # Option 2:
				# if alltime_rank == 'DNE':
				# 	if re.search(f'Top 2019 Movies', intlselector.xpath('.//td//text()').extract_first()) != None:
				# 	# if intlselector.xpath('./a/text()').extract_first()
				# 		alltime_rank = intlselector.xpath('./td[2]/text()').extract_first()
				# 		print('='*55)
				...
				# if year_rank == 'DNE': 
				# 	if re.search(f'Top 2019 Movies',intlselector.xpath('.//td//text()').extract_first()) != None:
				# 		year_rank = intlselector.xpath('./td[2]/text()').extract_first()
				# 		# try:
